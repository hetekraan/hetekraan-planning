...
// Canonieke categorie-waarden. Buslijst (lib/buslijst-build.js ALLOWED_CATEGORIES)
// en voorraad (public/app/planner-inventory.js groups) matchen exact op deze
// strings (hoofdletter + meervoud), zonder normalisatie. Een afwijkende waarde
// (kleine letter/enkelvoud) valt daar stil weg. Daarom mappen we hier bij het
// inlezen/opslaan alle bekende varianten naar de canonieke waarde.
export const CANONICAL_CATEGORIES = ['Kranen', 'Quookers', 'Serviceproducten', 'Diensten'];

const CATEGORY_ALIASES = {
  kranen: 'Kranen', kraan: 'Kranen',
  quookers: 'Quookers', quooker: 'Quookers',
  serviceproducten: 'Serviceproducten', serviceproduct: 'Serviceproducten', service: 'Serviceproducten',
  diensten: 'Diensten', dienst: 'Diensten',
};

/**
 * Mapt een binnenkomende categorie naar de canonieke waarde.
 * Herkent kleine letters, enkelvoud en varianten met/zonder spaties.
 * Onbekende waarde: onveranderd doorlaten (met console.warn), niet stil droppen,
 * zodat het opvalt i.p.v. later stilletjes de buslijst/voorraad te breken.
 * @param {unknown} raw
 * @returns {string}
 */
export function normalizeCategorie(raw) {
  const trimmed = String(raw ?? '').trim();
  if (!trimmed) return 'Serviceproducten';
  const lower = trimmed.toLowerCase();
  const compact = lower.replace(/\s+/g, '');
  const alias = CATEGORY_ALIASES[lower] || CATEGORY_ALIASES[compact];
  if (alias) return alias;
  console.warn('[prices] onbekende categorie, ongewijzigd doorgezet', { raw: trimmed });
  return trimmed;
}
...
function normalizePriceRow(row = {}) {
  ...
  const categorie = normalizeCategorie(row.categorie ?? row.category);
  ...
  const sortRaw = Number(row.sortOrder ?? row.sort_order);
  const sortOrder = Number.isFinite(sortRaw) ? sortRaw : null;
...
/**
 * Alleen-lezen: de ruwe, opgeslagen prijsrijen uit Redis (zonder normalisatie).
 * Handig om de daadwerkelijk opgeslagen categorie-waarde te inspecteren
 * (normalizePriceRow corrigeert immers bij het lezen via listPrices).
 */
export async function readRawPrices(locationId = 'default') {
  return readRaw(locationId);
}
...
export async function upsertPrice(locationId = 'default', row = {}) {
  ...
  if (i >= 0) {
    const merged = { ...rows[i], ...normalized, updatedAt: nowIso() };
    // Behoud bestaande sortOrder wanneer de PATCH er geen meestuurt (bijv. naam/prijs-edit),
    // zodat een gewone bewerking de handmatige volgorde niet wist.
    if (normalized.sortOrder == null && rows[i].sortOrder != null) merged.sortOrder = rows[i].sortOrder;
    rows[i] = merged;
  } else {
    rows.push({ ...normalized, updatedAt: nowIso() });
  }
...
/**
 * Atomische herordening: één read-modify-write voor de hele lijst.
 * Voorkomt de race die ontstaat wanneer de UI per rij een aparte PATCH stuurt
 * (elke upsertPrice herschrijft de hele array → last-write-wins wist sortOrder).
 * Kent sortOrder = index toe volgens de volgorde van orderedIds; overige rijen blijven ongemoeid.
 */
export async function reorderPrices(locationId = 'default', orderedIds = []) {
  ...
  for (const r of rows) {
    ...
    if (!orderMap.has(rid)) continue;
    const next = orderMap.get(rid);
    if (Number(r.sortOrder) !== next) {
      r.sortOrder = next;
      r.updatedAt = nowIso();
      changed += 1;
    }
  ...
  await writeRaw(locationId, rows);
  return { ok: true, changed, total: ids.length };
}
