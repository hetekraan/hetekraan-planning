...
export async function loadPlannerAppointmentsSource(input, deps) {
  ...
  try {
    ...
    const listPending = deps.cachedListPendingSyntheticEventsForDate;
    const [pendingSyn, confirmedSyn] = await Promise.all([
      typeof listPending === 'function'
        ? listPending(date).catch(() => [])
        : Promise.resolve([]),
      deps.cachedListConfirmedSyntheticEventsForDate(date),
    ]);
    blockBookingSynthetic = [
      ...(Array.isArray(pendingSyn) ? pendingSyn : []),
      ...(Array.isArray(confirmedSyn) ? confirmedSyn : []),
    ];
  ...
  for (const ev of blockBookingSynthetic) {
    ...
    events.push({
      ...
      _hkPendingBooking: Boolean(ev._hkPendingBooking),
    });
  }
