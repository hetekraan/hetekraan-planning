...
export function createSendNextEtaHandler(overrides = {}) {
  ...
  return async function sendNextEtaHandler(req, res) {
    ...
    const now = Date.now();
    const priorOrder = Array.isArray(loaded.routeState.orderContactIds)
      ? loaded.routeState.orderContactIds.map(cleanString).filter(Boolean)
      : [];
