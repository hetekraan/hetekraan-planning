...
function priceAndExtrasFromLines(totalAmountRaw, lines, row, fallback = null) {
  ...
  if (allExtras.length === 0) {
    price = Number.isFinite(effectiveTotal) && effectiveTotal >= 0 ? effectiveTotal : 0;
  } else if (Number.isFinite(effectiveTotal) && effectiveTotal >= 0 && effectiveTotal + 1e-6 >= extrasSum) {
    price = Math.max(0, Math.round((effectiveTotal - extrasSum) * 100) / 100);
  } else {
    price = 0;
  }
