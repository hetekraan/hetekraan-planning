 ...
 */
function priceAndExtrasFromLines(totalAmountRaw, lines) {
  const extras = (Array.isArray(lines) ? lines : []).map((ln) => {
    ...
    const tot = Number(ln?.total_price);
    let price = Number.isFinite(tot) ? tot : null;
    if (price == null && Number.isFinite(unit)) price = Math.round(unit * q * 100) / 100;
    if (price == null || !Number.isFinite(price)) price = 0;
    return { desc, price };
  });
  const extrasSum = extras.reduce((s, x) => s + (Number(x.price) || 0), 0);
  const totalCanon = parseFloat(String(totalAmountRaw ?? '').replace(/[^0-9.]/g, ''));
...
export async function loadSupabaseAnalyticsAppointments(supabaseUrl, serviceKey, startDate, endDate) {
  ...
  for (const row of appts) {
    ...
    const { price, extras } = priceAndExtrasFromLines(row.total_amount, lines);
    const addr =
