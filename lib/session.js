...
const SESSION_TTL_MS = 24 * 60 * 60 * 1000; // 24 uur

/**
 * Maak een ondertekend sessie-token geldig voor 24 uur.
 * Payload-formaat: user|expiresAtMs
 */
...
/**
 * Verifieer token. Geeft `{ user, expiresAt }` terug bij geldig token, anders `null`.
 * Ondersteunt ook het oude formaat user|YYYY-MM-DD voor backward-compatibiliteit.
 */
export function verifySessionToken(token) {
  ...
  // Nieuw formaat: second is een ms-timestamp
  const expiresAt = Number(second);
  if (!isNaN(expiresAt)) {
    if (Date.now() > expiresAt) return null; // verlopen
    return { user, expiresAt };
  }

  // Oud formaat (YYYY-MM-DD dag): accepteer nog voor bestaande tokens
  return { user, day: second };
}
