...
async function defaultOptimizeRoute({ appointments, preserveOrder }) {
  const mockReq = {
    method: 'POST',
    body: {
...
async function optimizeForRouteState({ activeAppointments, routeState, deps, preserveOrderIds = null }) {
  ...
  if (Array.isArray(preserveOrderIds)) {
    ...
    return deps.optimizeRoute({ appointments: ordered, preserveOrder: true });
  }

  const optimized = await deps.optimizeRoute({ appointments: activeAppointments, preserveOrder: false });
...
export function createReorderHandler(overrides = {}) {
  ...
  return async function reorderHandler(req, res) {
    ...
    if (staleIds.length) return json(res, 400, { ok: false, code: 'STALE_CONTACT_ID', staleContactIds: staleIds });
