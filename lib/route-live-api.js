...
export function routeStateConflictResponse(out) {
  return {
    ok: false,
    code: out.code,
...
export function validateBaseBody(body) {
...
export async function loadAndEnsureRouteState({ deps, req, locationId, dateStr }) {
  try {
...
export function mergeRoutePayload({ routeState, patch, expectedRevision, updatedBy, source }) {
  const now = Date.now();
  return {
    ...routeState,
    ...patch,
