...
import {
  listActiveSyntheticEventsForDate,
  listConfirmedSyntheticEventsForDate,
  listPendingSyntheticEventsForDate,
...
const activeSyntheticCacheKey = (dateStr) => `${amsterdamDayReadCacheKeyRedisSynthetics(dateStr)}:active`;
const pendingSyntheticCacheKey = (dateStr) => `${amsterdamDayReadCacheKeyRedisSynthetics(dateStr)}:pending`;

/** pending + confirmed — capaciteit / invite-validatie */
export async function cachedListActiveSyntheticEventsForDate(dateStr) {
  const key = activeSyntheticCacheKey(dateStr);
  const hit = amsterdamDayReadCacheGet(key);
  if (hit !== undefined) return hit;
  const fresh = await listActiveSyntheticEventsForDate(dateStr);
  const arr = Array.isArray(fresh) ? fresh : [];
  amsterdamDayReadCacheSet(key, arr);
  return arr;
}
...
export async function cachedListPendingSyntheticEventsForDate(dateStr) {
  const key = pendingSyntheticCacheKey(dateStr);
  const hit = amsterdamDayReadCacheGet(key);
  if (hit !== undefined) return hit;
  const fresh = await listPendingSyntheticEventsForDate(dateStr);
  const arr = Array.isArray(fresh) ? fresh : [];
  amsterdamDayReadCacheSet(key, arr);
  return arr;
}
...
export function invalidateRedisSyntheticsCacheForDate(dateStr) {
  ...
  store.delete(activeSyntheticCacheKey(ds));
  store.delete(pendingSyntheticCacheKey(ds));
}
