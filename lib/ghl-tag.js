...
/** GHL-workflow trigger voor pending boekingen (send-booking-invite). */
export const PENDING_BOOKING_TAG = 'niet-bevestigd';
 ...
 */
/**
 * Voeg een tag toe zonder delete/pulse — voorkomt onbedoeld opnieuw triggeren van andere tag-workflows.
 * @returns {Promise<boolean>}
 */
export async function addContactTag(contactId, tagName, logTag = '[ghl-tag]', hooks, options = {}) {
  const key = String(options.apiKey || process.env.GHL_API_KEY || '').trim();
  const locationId = String(process.env.GHL_LOCATION_ID || '').trim();
  const fetchFn = options.fetchFn || fetchWithRetry;
  ...
  try {
    emit('ghl_tag_add_request', { endpoint: tagUrl, method: 'POST' });
    const add = await fetchFn(tagUrl, {
      method: 'POST',
      headers,
      body: JSON.stringify({ tags: [tagName] }),
    });
    const addText = await add.text().catch(() => '');
    emit('ghl_tag_add_response', {
    ...
    if (!postOk) {
      emit('ghl_tag_add_failed', { httpStatus: add.status, reason: 'post_failed_or_body_rejects' });
      console.error(`${logTag} tag POST mislukt:`, add.status, summarizeTagResponseBody(addText));
      return false;
    }
    emit('ghl_tag_add_success', { httpStatus: add.status });
    return true;
  } catch (e) {
    emit('ghl_tag_add_failed', { message: e?.message || String(e) });
    console.error(`${logTag} addContactTag:`, e.message);
    return false;
  }
}
 ...
 * @param {string} tagName
 * @param {string} [logTag]
 * @param {{ on?: (event: string, payload: Record<string, unknown>) => void }} [hooks]
 * @param {{ apiKey?: string, fetchFn?: typeof fetchWithRetry }} [options]
 * @returns {Promise<boolean>}
 */
export async function removeContactTag(contactId, tagName, logTag = '[ghl-tag]', hooks, options = {}) {
  const key = String(options.apiKey || process.env.GHL_API_KEY || '').trim();
  const locationId = String(process.env.GHL_LOCATION_ID || '').trim();
  const fetchFn = options.fetchFn || fetchWithRetry;
  const tagUrl = `${GHL_BASE}/contacts/${contactId}/tags`;
  ...
  try {
    emit('ghl_tag_remove_request', { endpoint: tagUrl, method: 'DELETE' });
    const del = await fetchFn(tagUrl, {
      method: 'DELETE',
      headers,
      body: JSON.stringify({ tags: [tagName] }),
    });
    const delText = await del.text().catch(() => '');
    emit('ghl_tag_remove_response', {
      httpStatus: del.status,
      responseBody: summarizeTagResponseBody(delText),
    });
    if (del.status === 401 || del.status === 403) {
      emit('ghl_tag_remove_failed', { httpStatus: del.status, reason: 'delete_unauthorized' });
      console.error(`${logTag} tag DELETE auth mislukt:`, del.status, summarizeTagResponseBody(delText));
      return false;
    }
    if (!del.ok && del.status !== 404) {
      emit('ghl_tag_remove_failed', { httpStatus: del.status, reason: 'delete_failed' });
      console.warn(`${logTag} tag DELETE mislukt:`, del.status, summarizeTagResponseBody(delText));
      return false;
    }
    emit('ghl_tag_remove_success', { httpStatus: del.status });
    return true;
  } catch (e) {
    emit('ghl_tag_remove_failed', { message: e?.message || String(e) });
    console.warn(`${logTag} removeContactTag:`, e.message);
    return false;
  }
}

export async function pulseContactTag(contactId, tagName, logTag = '[ghl-tag]', hooks, options = {}) {
  const key = String(options.apiKey || process.env.GHL_API_KEY || '').trim();
  const locationId = String(process.env.GHL_LOCATION_ID || '').trim();
  const fetchFn = options.fetchFn || fetchWithRetry;
  const delayMs =
    Number.isFinite(options.delayMs) && options.delayMs >= 0
      ? Math.min(Math.round(options.delayMs), 10000)
      : 1500;
  const tagUrl = `${GHL_BASE}/contacts/${contactId}/tags`;
  if (!key || !contactId || !tagName) return false;
...
/** @returns {Promise<boolean>} */
export function pulsePendingBookingTag(contactId, logTag = '[ghl-tag]', hooks, options = {}) {
  return pulseContactTag(contactId, PENDING_BOOKING_TAG, logTag, hooks, {
    delayMs: 2000,
    ...options,
  });
}
...
/** @returns {Promise<boolean>} */
export function removePendingBookingTag(contactId, logTag = '[ghl-tag]', hooks, options = {}) {
  return removeContactTag(contactId, PENDING_BOOKING_TAG, logTag, hooks, options);
}
