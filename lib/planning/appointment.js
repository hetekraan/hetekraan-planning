...
/**
 * Leest één contact custom field op id — zelfde patroon als api/ghl `getField` (value + field_value, id als string).
 */
export function readContactCustomFieldById(contact, fieldId) {
...
export function plannerServiceMarkedCompleteOnRouteDay(datumOnderhoudFieldValue, routeYmd) {
  ...
  const isoIn = v.match(/(\d{4}-\d{2}-\d{2})/);
  if (isoIn && isoIn[1] === r) return true;
  /** NL-notatie (dag-maand-jaar) komt in GHL/custom fields vaker voor dan ISO. */
  const dmy = v.match(/^(\d{1,2})[./-](\d{1,2})[./-](\d{4})(?:\b|T|\s|$)/);
  if (dmy) {
    const dd = String(parseInt(dmy[1], 10)).padStart(2, '0');
    const mm = String(parseInt(dmy[2], 10)).padStart(2, '0');
    const yyyy = dmy[3];
    if (`${yyyy}-${mm}-${dd}` === r) return true;
  }
  return false;
}

function paymentStatusLooksAfgerond(s) {
  const t = String(s || '').trim().toLowerCase();
  if (!t) return false;
  return t.includes('afgerond') || t.includes('betaald') || t === 'paid';
}
...
export function mapEnrichedGhlEventToAppointment(e, rowFallbackIndex = 0, viewedRouteDateStr = '') {
  ...
  const datumInstallatieVal = readContactCustomFieldById(contact, CF_DATUM_INSTALLATIE);
  const datumOnderhoudVal = readContactCustomFieldById(contact, CF_DATUM_ONDERHOUD);
  const legacyBetalingsstatus = readContactCustomFieldById(
    contact,
    LEGACY_COMPLETE_FIELD_IDS.legacy_betalingsstatus
  );
  ...
  const hasValidTotal = Number.isFinite(totalCanon);
  ...
  } else if (hasValidTotal) {
    price = Math.round((totalCanon - extrasSum) * 100) / 100;
  } else {
    price = 0;
  }

  let status = 'ingepland';
  /** Bron van waarheid: GHL `datum_laatste_onderhoud` = route-dag (completeAppointment schrijft dit). */
  const datumMatchesRoute =
    Boolean(viewedRouteDateStr) &&
    plannerServiceMarkedCompleteOnRouteDay(datumOnderhoudVal, viewedRouteDateStr);
  const paymentClosed =
    paymentStatusLooksAfgerond(e.parsedPaymentStatus) || paymentStatusLooksAfgerond(legacyBetalingsstatus);
  if (!isCalBlock && viewedRouteDateStr && datumMatchesRoute) {
    status = 'klaar';
  } else if (!isCalBlock && viewedRouteDateStr && paymentClosed && !datumOnderhoudVal) {
    /** Fallback: alleen betaalstatus zichtbaar / datumveld leeg na sync — niet impliciet terug naar open. */
    status = 'klaar';
  }
  ...
  const isPendingBooking = Boolean(e._hkPendingBooking);
  const timeSlotForCard =
    isPendingBooking && !isCalBlock
      ? dayPart === 1
        ? SLOT_LABEL_AFTERNOON_NL
        : SLOT_LABEL_MORNING_NL
      : timeSlot;
  ...
  return {
    ...
    isPendingBooking: Boolean(e._hkPendingBooking),
    name,
    address,
    straatnaam: straat,
    huisnummer: huisnr,
    ...
    ...(e.invoiceFields && typeof e.invoiceFields === 'object' ? e.invoiceFields : {}),
  };
}
