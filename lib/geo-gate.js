...
const DEPOT_MAX_MIN = envNumber('PROPOSAL_DEPOT_MAX_MIN', 45);
 ...
 */
export function isGeoValid(newCoord, dayContext) {
  ...
  if (morning.length === 0 && afternoon.length === 0) {
    ...
    if (depotMin > DEPOT_MAX_MIN) return { valid: false, reason: 'depot-too-far' };
  }
  ...
  if (targetCentroid) {
    ...
    if (km > BLOCK_RADIUS_KM) return { valid: false, reason: 'block-centroid-exceeded' };
  }
  ...
  if (otherCentroid) {
    ...
    if (transitionMin > BLOCK_TRANSITION_MIN) {
