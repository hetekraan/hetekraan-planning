 ...
 */
export function appendNotionPlannerNote(existingNotes, { status, klusId, url } = {}) {
  const base = String(existingNotes || '')
    .replace(/\[notion\]\s*[^\n\r]*/gi, '')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
  const parts = [`status=${String(status || 'pending').trim()}`];
...
/** Parse de `[notion]`-marker uit notitietekst. Return null als afwezig. */
export function parseNotionPlannerNote(notes) {
  const raw = String(notes || '');
  const m = raw.match(/\[notion\][^\n\r]*/i);
  if (!m) return null;
  const line = m[0];
  ...
  return { status, klusId, url };
}

export function isNotionConfigured() {
  return Boolean(
    String(process.env.NOTION_TOKEN || '').trim() &&
      String(process.env.NOTION_DB_KLANTEN || '').trim() &&
      String(process.env.NOTION_DB_KLUSSEN || '').trim()
  );
}
...
/**
 * TIJDELIJKE DIAGNOSTIEK (verwijderen na diagnose van "Could not find property GHL-ID").
 * Logt de exacte query-config + haalt read-only het Klanten-db-schema op en print de
 * property-namen mét hex/codepoints, zodat verborgen unicode (bijv. non-breaking hyphen
 * U+2011 vs normale hyphen U+002D) of afwijkende spelling zichtbaar wordt.
 */
async function logKlantenSchemaDiagnostics(dbId, klantData, deps) {
  try {
    console.info(
      '[notion][diag] klanten_query_config',
      JSON.stringify({
        NOTION_DB_KLANTEN: dbId,
        ghlIdProperty: NOTION_KLANT_PROPS.ghlId,
        telefoonProperty: NOTION_KLANT_PROPS.telefoon,
        ghlIdFilterBody: {
          filter: { property: NOTION_KLANT_PROPS.ghlId, rich_text: { equals: notionText(klantData?.ghlId) } },
          page_size: 1,
        },
      })
    );
    const schema = await notionRequest(`/databases/${encodeURIComponent(dbId)}`, { deps });
    const props = schema?.properties && typeof schema.properties === 'object' ? schema.properties : {};
    const names = Object.keys(props);
    const detail = names.map((name) => ({
      name,
      type: props[name]?.type || null,
      hex: Buffer.from(name, 'utf8').toString('hex'),
      codePoints: Array.from(name).map((ch) => ch.codePointAt(0)),
    }));
    console.info(
      '[notion][diag] klanten_db_schema',
      JSON.stringify({
        dbId,
        title: Array.isArray(schema?.title) ? schema.title.map((t) => t?.plain_text || '').join('') : null,
        propertyNames: names,
        expectedGhlIdHex: Buffer.from(NOTION_KLANT_PROPS.ghlId, 'utf8').toString('hex'),
        properties: detail,
      })
    );
  } catch (diagErr) {
    console.warn('[notion][diag] schema_fetch_failed', {
      dbId,
      status: diagErr?.status ?? null,
      code: diagErr?.code || null,
      message: diagErr?.message ? String(diagErr.message).slice(0, 300) : String(diagErr),
    });
  }
}
...
export async function upsertKlantInNotion(klantData = {}, deps) {
  const dbId = String(process.env.NOTION_DB_KLANTEN || '').trim();
  if (!dbId) {
    const err = new Error('NOTION_DB_KLANTEN ontbreekt');
    err.code = 'notion_not_configured';
    throw err;
  }
  await logKlantenSchemaDiagnostics(dbId, klantData, deps);
...
export async function createKlusInNotion(klusData = {}, notionKlantPageId, deps) {
  const dbId = String(process.env.NOTION_DB_KLUSSEN || '').trim();
  if (!dbId) {
