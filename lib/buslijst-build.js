...
/** @typedef {{ byId: Map<string, PriceRow>, bySku: Map<string, PriceRow>, byNameNormalized: Map<string, PriceRow> }} PriceCatalogMaps */
 ...
 * @returns {string}
 */
function lineLabelForCatalogMatch(line) {
  for (const key of ['desc', 'naam', 'name', 'label', 'description']) {
...
export function buildPriceCatalogMaps(priceRows) {
  ...
  for (const row of Array.isArray(priceRows) ? priceRows : []) {
    ...
    const nameKey = normalizeProductName(row?.naam || row?.name);
    if (!nameKey) continue;
    if (byNameNormalized.has(nameKey)) {
      const existing = byNameNormalized.get(nameKey);
      const existingId = String(existing?.id || '').trim();
      const rowId = String(row?.id || '').trim();
      if (existingId !== rowId) {
        console.warn('[buslijst] duplicate_product_name', {
          nameKey,
          firstId: existingId,
          skippedId: rowId,
        });
    ...
    byNameNormalized.set(nameKey, row);
  }
  return { byId, bySku, byNameNormalized };
}

/**
 * Match: priceId → sku → exacte naam (case-insensitive, whitespace genormaliseerd).
 * Geen fuzzy/partiële match.
 * @param {Record<string, unknown>} line
...
export function resolveCatalogProductStrict(line, maps) {
  ...
  const nameKey = normalizeProductName(lineLabelForCatalogMatch(line));
  if (nameKey && maps.byNameNormalized.has(nameKey)) {
    return maps.byNameNormalized.get(nameKey) || null;
  }
