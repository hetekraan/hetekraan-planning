 ...
 * @param {string|null} [opts.assignedUserId] — query blocked-slots voor deze user (personal calendar blocks).
 */
export async function fetchBlockedSlotsAsEvents(
  ...
  const runFetch = async (version, includeCal) => {
    ...
    if (uid) url += `&assignedUserId=${encodeURIComponent(uid)}`;
...
export async function listBlockedSlotIdsForRange(
  base,
  { locationId, calendarId, apiKey, startMs, endMs, assignedUserId }
) {
  if (!apiKey || !locationId || startMs == null || endMs == null) return [];
 ...
 * @param ctx.assignedUserId — optioneel; blocked-slots voor personal calendar (naast calendarId).
 */
