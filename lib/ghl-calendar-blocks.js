...
export async function postFullDayBlockSlot(
  ...
  if (process.env.GHL_BLOCK_SLOT_SKIP_IDEMPOTENCY_CHECK !== 'true') {
    const hasOnCalendar = await dayHasBlockedSlotsOverlappingWorkHours(
      base,
    ...
    if (hasOnCalendar) {
      return {
        ...
        data: {},
        detail:
          'Deze kalender heeft al bloktijd die de werkdag raakt — er is geen extra blokslot gezet. Ruim overtollige blokken zo nodig op in GHL.',
      };
    }
  }
