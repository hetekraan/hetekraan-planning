...
/** POST block-slots: marketplace-doc gebruikt 2021-04-15; eerst proberen voorkomt 422 “calendarId/user” op sommige tenants. */
const BLOCK_SLOT_POST_VERSIONS = ['2021-04-15', '2021-07-28'];
...
export async function postFullDayBlockSlot(
  ...
  /** Altijd calendarId + assignedUserId — nooit POST zonder kalender (geen user-only spookblokken op Jerry). */
  const baseBody = {
    locationId: loc,
    calendarId: cal,
    title: titleStr,
  ...
  if (endExclusiveZ) {
    shapes.push({
      ...
      body: { ...baseBody, startTime: startIsoZ, endTime: endExclusiveZ },
    });
  }
  shapes.push({
    label: 'iso-z-inclusive-day-end',
    body: { ...baseBody, startTime: startIsoZ, endTime: endInclusiveZ },
  });
  if (startAms && endNextAms) {
    shapes.push({
      label: 'ams-offset-next-midnight',
      body: { ...baseBody, startTime: startAms, endTime: endNextAms },
    });
  }
  if (startAms && endAms) {
    shapes.push({
      label: 'ams-offset-same-day',
      body: { ...baseBody, startTime: startAms, endTime: endAms },
    });
  }
  ...
  for (const { label, body } of shapes) {
    for (const version of BLOCK_SLOT_POST_VERSIONS) {
