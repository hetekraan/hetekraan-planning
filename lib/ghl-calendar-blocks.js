...
export async function postFullDayBlockSlot(
  ...
  /** ISO (zoals appointments); daarna epoch-ms als string (sommige GHL-validators); laatste redmiddel zonder calendarId. */
  const shapes = [
    {
      label: 'iso',
      build: (includeCal) => {
  ...
  const phases = [true];
  if (process.env.GHL_BLOCK_SLOT_TRY_NO_CALENDAR_ID === 'true') phases.push(false);
