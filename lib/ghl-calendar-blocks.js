...
export async function postFullDayBlockSlot(
  ...
  /** GHL verwacht hier offset-notatie (+02:00), niet UTC-Z — anders personal calendar block-slots falen. */
  const startIsoZ = startD.toISOString().replace('Z', '+02:00');
  const endInclusiveZ = new Date(bounds.endMs).toISOString().replace('Z', '+02:00');
  ...
  const endExclusiveZ = endNextMid ? endNextMid.toISOString().replace('Z', '+02:00') : null;

  /** Twee tijd-varianten (exclusief/inclusief eind), zelfde offset als start. */
  const timeAttempts = [];
  if (endExclusiveZ) {
  ...
  for (const { label, endTime } of timeAttempts) {
    ...
    console.log('[postFullDayBlockSlot] cal (idempotency only):', cal, '| uid:', uid, '| loc:', loc);
    const body = {
      assignedUserId: uid,
      locationId: loc,
      startTime,
