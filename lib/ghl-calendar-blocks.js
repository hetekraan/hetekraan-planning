 ...
 *   Gebruik voor idempotency bij “blokkeer dag” op Planning Jerry.
 */
export async function fetchBlockedSlotsAsEvents(
  base,
  { locationId, calendarId, startMs, endMs, apiKey, calendarScopedOnly = false }
) {
  ...
  try {
    for (const version of versions) {
      ...
      if (cal && !calendarScopedOnly) await runFetch(version, false);
    }
  } catch {
...
export async function dayHasBlockedSlotsOverlappingWorkHours(
  base,
  { locationId, calendarId, apiKey, calendarScopedOnly = false },
  dateStr
) {
...
/**
 * GET /calendars/events voor deze kalenderdag: blok-achtige items die werktijd 09–17 Amsterdam raken?
...
/**
 * Hele Amsterdam-kalenderdag blokkeren op één kalender (GHL_CALENDAR_ID).
 * POST bevat altijd calendarId + assignedUserId — geen fallback zonder kalender (voorkomt user-only spookblokken).
 */
export async function postFullDayBlockSlot(
  ...
  /**
   * Voorkom dubbele “Blokkeer dag”-klikken: alleen blocked-slots **met deze calendarId**
   * (niet user-wide zonder kalender — dat voorkwam vroeger dubbele POST + user-only blokken).
   */
  if (process.env.GHL_BLOCK_SLOT_SKIP_IDEMPOTENCY_CHECK !== 'true') {
    const hasBlockedSlots = await dayHasBlockedSlotsOverlappingWorkHours(
      base,
      { locationId: loc, calendarId: cal, apiKey, calendarScopedOnly: true },
      dateStr
    );
  ...
  /**
   * Twee tijd-varianten (exclusief/inclusief eind), alleen ISO-Z — geen ams-offset meer in logs.
   * Body: calendarId = door caller doorgegeven `cal` (effectiveCalendarId in api/ghl.js), geen dubbele env-lees.
   */
  const timeAttempts = [];
  if (endExclusiveZ) {
    timeAttempts.push({ label: 'iso-z-exclusive-next-mid', endTime: endExclusiveZ });
  }
  timeAttempts.push({ label: 'iso-z-inclusive-day-end', endTime: endInclusiveZ });

  let last = { ok: false, status: 0, data: {}, detail: 'Geen poging uitgevoerd' };

  for (const { label, endTime } of timeAttempts) {
    const calId = cal; // altijd parameter van caller, niet process.env opnieuw
    console.log('[DEBUG] calendarId value:', calId);
    console.log('[DEBUG] typeof:', typeof calId);

    const startTime = startIsoZ;
    console.log('[postFullDayBlockSlot] cal param:', cal, '| uid:', uid, '| loc:', loc);
    const body = {
      calendarId: cal,
      startTime,
      endTime,
      title: titleStr,
      /** Vereist door GHL voor jullie tenant (422 zonder): */
      locationId: loc,
      assignedUserId: uid,
    };
...
export async function deleteGhlCalendarBlock(base, apiKey, eventId) {
  ...
  /** Eerst native event-DELETE (GHL UI sync / block-slots wijzen vaak naar hetzelfde event-id). */
  const urls = [
...
/**
 * Unieke echte blokslot-ids in een ms-bereik (blocked-slots: mét én zonder calendarId
 * zodat ook user-only blokken uit het dashboard meekomen).
 */
export async function listBlockedSlotIdsForRange(
  base,
  { locationId, calendarId, apiKey, startMs, endMs }
) {
  if (!apiKey || !locationId || startMs == null || endMs == null) return [];
...
export async function listDeletableBlockIdsForMsRange(
  ...
  const ids = new Set(
    await listBlockedSlotIdsForRange(base, { locationId, calendarId, apiKey, startMs, endMs })
  );
  const cal = calendarId != null && String(calendarId).trim() ? String(calendarId).trim() : '';
  if (cal && locationId) {
