...
/**
 * Eén keer blocked-slots + kalender-events over [rangeStartDateStr … rangeEndDateStr] (Amsterdam-dagen),
 * daarna Set van datums (YYYY-MM-DD) waar dezelfde logica als dayHasCustomerBlockingOverlap true is.
 * Gebruikt door suggest-slots i.p.v. ~30× aparte GHL-roundtrips (voorkomt 429 Too Many Requests).
 */
export async function blockedCustomerDatesInAmsterdamRange(
  base,
  { locationId, calendarId, apiKey, assignedUserId = null },
  rangeStartDateStr,
  rangeEndDateStr
  ...
  const out = new Set();
  const startBounds = amsterdamCalendarDayBoundsMs(rangeStartDateStr);
  const endBounds = amsterdamCalendarDayBoundsMs(rangeEndDateStr);
  if (!startBounds || !endBounds || !apiKey || !locationId) return out;
  const startMs = startBounds.startMs;
  const endMs = endBounds.endMs;
  const cal = calendarId != null && String(calendarId).trim() ? String(calendarId).trim() : '';
  if (!cal) return out;
  ...
  let calendarEvents = [];
  const evUrl = `${base}/calendars/events?locationId=${encodeURIComponent(locationId)}&calendarId=${encodeURIComponent(cal)}&startTime=${startMs}&endTime=${endMs}`;
  for (const Version of ['2021-04-15', '2021-07-28']) {
    const res = await fetchWithRetry(
      evUrl,
      { headers: { Authorization: `Bearer ${apiKey}`, Version } },
  ...
  const events = [...calendarEvents, ...blockedMerged];
  markBlockLikeOnCalendarEvents(events);

  let cursor = rangeStartDateStr;
  for (let guard = 0; guard < 400 && cursor; guard++) {
    if (cursor > rangeEndDateStr) break;
    const dayBounds = amsterdamCalendarDayBoundsMs(cursor);
    if (!dayBounds) break;
    const workStart = amsterdamWallTimeToDate(cursor, WORK_DAY_START_HOUR, 0)?.getTime();
    const workEnd = amsterdamWallTimeToDate(cursor, WORK_DAY_END_HOUR, 0)?.getTime();
    ...
    for (const e of events) {
      ...
      if (Number.isNaN(eMs)) eMs = dayBounds.endMs;
    ...
    if (
      !blocked &&
      process.env.GHL_SKIP_LONG_HOLD_HEURISTIC !== 'true'
    ) {
      const minHoldMs = 6 * 3600000;
      for (const e of events) {
        if (e._hkGhlBlockSlot) continue;
    ...
    if (blocked) out.add(cursor);
    const next = addAmsterdamCalendarDays(cursor, 1);
