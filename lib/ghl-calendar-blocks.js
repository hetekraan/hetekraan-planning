...
export async function fetchBlockedSlotsAsEvents(
  ...
  try {
    for (const version of versions) {
      ...
      } else if (cal && calendarScopedOnly) {
        await runFetchQuery(version, `calendarId=${encodeURIComponent(cal)}`);
      } else {
        if (cal) await runFetchQuery(version, `calendarId=${encodeURIComponent(cal)}`);
        if (cal && !calendarScopedOnly) await runFetchQuery(version, '');
      }
    }
  } catch {
...
export async function deleteGhlCalendarBlock(base, apiKey, eventId) {
  ...
  const urls = [
    ...
    `${base}/calendars/blocked-slots/${enc}`,
 ...
 * Blocked-slots worden via **userId** (assignedUserId in ctx) opgehaald — niet via calendarId,
 * omdat personal block-slots zonder event-calendarId in GHL staan.
 */
