...
export async function postFullDayBlockSlot(
   ...
   * Body: calendarId uit env (debug) met fallback naar door caller doorgegeven `cal` als env leeg is
   * (anders mist JSON key en ziet GHL geen calendarId).
   */
  const timeAttempts = [];
  if (endExclusiveZ) {
  ...
  for (const { label, endTime } of timeAttempts) {
    ...
    const body = {
      calendarId: stripGhlEnvIdLocal(calId) || cal,
      startTime,
      endTime,
      title: titleStr,
