...
/** Ruwe validatie voor suggest/invite (zelfde idee als book.html). */
export function isValidPlainEmail(raw) {
  const s = normalizeCanonicalGhlEmail(raw);
  if (!s || s.length > 254) return false;
  return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i.test(s);
}

/**
 * `GET /contacts/search/duplicate` — response-shape verschilt (contact-wrapper vs. id).
 * Zelfde fallbacks als createAppointment in api/ghl.js.
 * @param {unknown} payload
 * @returns {string|null}
 */
export function readGhlDuplicateSearchContactId(payload) {
  if (!payload || typeof payload !== 'object') return null;
  const id = payload.contact?.id ?? payload.id ?? payload.data?.contact?.id ?? null;
...
export function buildCanonicalAddressWritePayload(fullLine, extra = {}) {
  ...
  // Per-veld schrijven (idempotent): straatnaam-CF bevat alleen de straat, niet de
  // hele regel. Anders dupliceert readCanonicalAddressLine postcode/plaats bij elke
  // read+write-cyclus (zie fix(customers): address corruption root-fix).
  add(GHL_ADDR_CF_IDS.straatnaam, straatnaam);
  add(GHL_ADDR_CF_IDS.huisnummer, huisnummer);
  add(GHL_ADDR_CF_IDS.postcode, postcode);
  add(GHL_ADDR_CF_IDS.woonplaats, woonplaats);
