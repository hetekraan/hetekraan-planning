...
function normalizeViolationEntry(value) {
  if (!value || typeof value !== 'object') return null;
  const kind = cleanString(value.kind).toLowerCase();
  if (kind !== 'time_window' && kind !== 'internal_fixed') return null;
  const out = { kind };
  const constraint = cleanString(value.constraint).toLowerCase();
  if (constraint) out.constraint = constraint;
  const reason = cleanString(value.reason);
  if (reason) out.reason = reason;
  const eta = normalizeTimeStr(value.eta);
  if (eta) out.eta = eta;
  const fixedTime = normalizeTimeStr(value.fixedTime);
  if (fixedTime) out.fixedTime = fixedTime;
  const finishesAt = normalizeTimeStr(value.finishesAt);
  if (finishesAt) out.finishesAt = finishesAt;
  const earliest = normalizeTimeStr(value.earliest);
  if (earliest) out.earliest = earliest;
  const waitMinutes = Number(value.waitMinutes);
  if (Number.isFinite(waitMinutes) && waitMinutes > 0) out.waitMinutes = Math.floor(waitMinutes);
  const window = cleanString(value.window);
  if (window) out.window = window;
  return out;
}

function normalizeViolationsMap(raw, allowedIds) {
  if (!raw || typeof raw !== 'object') return {};
  const allowed = allowedIds ? new Set(allowedIds) : null;
  const out = {};
  for (const [key, value] of Object.entries(raw)) {
    ...
    const entry = normalizeViolationEntry(value);
    if (entry) out[contactId] = entry;
  }
  return out;
}
...
export function normalizeRouteLivePayload(input) {
  ...
  const violationsByContactId = normalizeViolationsMap(input.violationsByContactId, orderContactIds);
