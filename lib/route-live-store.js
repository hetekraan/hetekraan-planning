...
import { getRouteLock } from './route-lock-store.js';
...
function routeLivePayloadFromLegacyLock(locationId, dateStr, legacyLock, appointments) {
  const orderContactIds = normalizeContactIdList(legacyLock?.orderContactIds);
  ...
  return {
    ...
    routeStatus: 'live',
    orderContactIds,
    etasByContactId: legacyLock?.etasByContactId || {},
    internalFixedStartByContactId: legacyLock?.internalFixedStartByContactId || {},
    lastOptimizedAt: now,
...
export async function ensureRouteLiveState(locationId, dateStr, appointments) {
  ...
  const legacyLock = await getRouteLock(loc, ds);
  const payload =
    legacyLock?.locked === true
      ? routeLivePayloadFromLegacyLock(loc, ds, legacyLock, appointments)
      : routeLivePayloadFromAppointments(ds, appointments);
