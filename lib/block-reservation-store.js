...
function keyUniq(contactId, dateStr) {
  return `${redisPrefix()}${BLOCK_RESERVATION_KEY_PREFIX}:uniq:${contactId}:${dateStr}`;
}

/** Volledige record JSON: `hk:block_res:data:{id}` */
function keyData(id) {
  return `${redisPrefix()}${BLOCK_RESERVATION_KEY_PREFIX}:data:${id}`;
}

/** Set van alle reservation-ids op die kalenderdag: `hk:block_res:day:{dateStr}` */
function keyDay(dateStr) {
  return `${redisPrefix()}${BLOCK_RESERVATION_KEY_PREFIX}:day:${dateStr}`;
}
...
export async function listReservationsForContact(contactId) {
  ...
  try {
    const pattern = `${redisPrefix()}${BLOCK_RESERVATION_KEY_PREFIX}:uniq:${cid}:*`;
    const results = [];
    let cursor = 0;
    ...
    do {
      let scanOut;
      try {
        // Upstash redis-sdk v1 ondersteunt object-opties.
        scanOut = await redis.scan(cursor, { match: pattern, count: 100 });
      } catch {
        // Compat fallback voor clients met Redis-argstijl.
        scanOut = await redis.scan(cursor, 'MATCH', pattern, 'COUNT', 100);
      }
      ...
      for (const keyRaw of keys) {
        ...
        const raw = await redis.get(keyData(reservationId));
        if (raw == null) continue;
        try {
          ...
          results.push({
            contactId: cid,
            dateStr,
            block: reservation?.block || null,
            workType: reservation?.workType || null,
            id: reservationId,
          });
        } catch {
          // Corrupte JSON overslaan.
        }
      }
