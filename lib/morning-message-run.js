...
import {
  countMorningEligibleAppointments,
  isPlannerAppointmentEligibleForMorningMessage,
} from './planner-appointment-status.js';
...
export async function runMorningMessagesForDay(input) {
  ...
  const ingepland = appointments.filter((a) => isPlannerAppointmentEligibleForMorningMessage(a));
  console.log('[morning-debug] runMorningMessagesForDay after load', {
    dateStr,
    locationId,
    totalAppointments: appointments.length,
    eligibleCount: ingepland.length,
    appointmentShapes: appointments.map((a) => ({
      id: a?.id,
      contactId: a?.contactId,
      firstName: a?.firstName ?? a?.name,
      status: a?.status,
      appointmentStatus: a?.appointmentStatus,
      jobType: a?.jobType,
      isCalBlock: a?.isCalBlock,
      fullAddressLine: a?.fullAddressLine ? '(set)' : '(missing)',
      eligibleByHelper: isPlannerAppointmentEligibleForMorningMessage(a),
    })),
  });
  if (!ingepland.length) {
    console.log('[morning-debug] skip NO_INGEPLAND — zero eligible after helper filter');
  ...
  console.log('[morning-debug] after route filter buildMorningMessageAppointmentsForIngepland', {
    dateStr,
    orderContactIds: routeState?.orderContactIds ?? [],
    eligibleBeforeRouteFilter: ingepland.length,
    payloadCount: payload.length,
    payloadContactIds: payload.map((r) => r.contactId),
  });
  if (!payload.length) {
    console.log('[morning-debug] skip NO_PAYLOAD — eligible appointments not in route orderContactIds');
...
export function countIngeplandAppointments(appointments) {
  return countMorningEligibleAppointments(appointments);
}
