...
import {
  ...
  logEnforceMorningBeforeAfternoonOrder,
  logRouteOptimizeInput,
  logRouteOptimizeResult,
...
export async function defaultOptimizeRoute({ appointments, preserveOrder }) {
  logRouteOptimizeInput(appointments, '[route-optimizer] optimize_input', null);
  ...
  logRouteOptimizeResult(orderContactIds, appointments, '[route-optimizer] optimize_result');
  return {
    orderContactIds,
...
function logOptimizeForRouteStateInput(activeAppointments, routeState, preserveOrderIds) {
  logRouteOptimizeInput(
    activeAppointments,
    '[route-optimizer] optimize_for_route_state_input',
    routeState?.pinsByContactId
  );
  console.info(
    '[route-optimizer] optimize_for_route_state_context',
    JSON.stringify({
      revision: routeState?.revision ?? null,
      source: cleanString(routeState?.source) || null,
      preserveOrder: Array.isArray(preserveOrderIds),
      orderContactIds: Array.isArray(routeState?.orderContactIds) ? routeState.orderContactIds : [],
    })
  );
}
...
export async function optimizeForRouteState({
  ...
  logOptimizeForRouteStateInput(activeAppointments, routeState, preserveOrderIds);
  ...
  if (Array.isArray(preserveOrderIds)) {
    ...
    logEnforceMorningBeforeAfternoonOrder(
      ...
      enforcedIds,
      activeAppointments,
      '[route-optimizer] enforce_on_preserve_input'
    );
    ...
    const before = Array.isArray(plan.orderContactIds) ? plan.orderContactIds : [];
    plan.orderContactIds = enforceMorningBeforeAfternoonOrder(before, activeAppointments);
    logEnforceMorningBeforeAfternoonOrder(
      before,
      plan.orderContactIds,
      activeAppointments,
      '[route-optimizer] enforce_on_preserve_output'
    );
    logRouteOptimizeResult(
      plan.orderContactIds,
      activeAppointments,
      '[route-optimizer] optimize_for_route_state_result'
    );
    return plan;
  }
  ...
  logEnforceMorningBeforeAfternoonOrder(
    ...
    plan.orderContactIds,
    activeAppointments,
    '[route-optimizer] enforce_on_reoptimize_output'
  );
  logRouteOptimizeResult(
    plan.orderContactIds,
    activeAppointments,
...
export async function triggerLiveRouteRecalculation({
  ...
  try {
    ...
    if (dayPartViolation) {
      console.info(
        'route_live_day_part_order_violation',
        JSON.stringify({
          reason: why,
          date,
          locationId: loc,
          revision: ensured.routeState.revision ?? null,
          orderContactIds: ensured.routeState.orderContactIds || [],
        })
      );
    }
