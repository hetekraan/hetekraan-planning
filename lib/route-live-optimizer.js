...
export async function triggerLiveRouteRecalculation({
  ...
  try {
    ...
    // createAppointment + reschedule: volledige heroptimalisatie (zelfde pad als
    // manual_button), niet preserveOrder — stop mag mid-route; pins via
    // applyPinnedPositions. Bewust NIET voor complete / setInternalFixedStart /
    // day_part_repair.
    const freeFullReoptimize =
      why === 'createAppointment' ||
      why === 'rescheduleAppointment_prevDate' ||
      why === 'rescheduleAppointment_newDate';
    const preservedOrder =
      repairDayPart || freeFullReoptimize
        ? null
