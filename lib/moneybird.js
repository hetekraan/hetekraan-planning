...
/**
 * Moneybird 422: onbereikbaar e-maildomein → retry zonder e-mail op contact.
 * @param {unknown} err
 */
export function isMoneybirdEmailDomainUnreachableFailure(err) {
  const st = err && typeof err === 'object' && 'status' in err ? err.status : null;
  if (Number(st) !== 422) return false;
  ...
  const det =
    err && typeof err === 'object' && 'details' in err
      ...
      : '';
  const blob = `${msg}\n${det}`.toLowerCase();
  if (blob.includes('email_domain_unreachable')) return true;
  if (blob.includes('send_invoices_to_email') && (blob.includes('cannot receive') || blob.includes('unreachable')))
    return true;
  return false;
}
...
export async function mbRequest(path, options = {}) {
  if (!MB_TOKEN || !MB_ADMIN) {
...
export async function createContact({ name, email, phone, address, invoiceParty, omitEmail } = {}) {
  ...
  try {
    ...
    const emailDomainUnreachable = isMoneybirdEmailDomainUnreachableFailure(err);
    console.error(
      '[moneybird] contact_create_failed',
      JSON.stringify({
        status: err?.status,
        message: String(err?.message || err).slice(0, 400),
        emailDomainUnreachable,
      })
    );
    const details =
      err && typeof err === 'object' && 'details' in err
    ...
    return {
      ...
      errorMessage: String(err?.message || err).slice(0, 800),
      errorDetails: String(details).slice(0, 2000),
    };
  }
}
...
export async function findOrCreateContact(name, email, phone, address, options = {}) {
  ...
  if (created?.emailDomainUnreachable) {
    const retry = await createContact({ name, email, phone, address, invoiceParty, omitEmail: true });
    if (retry?.created && retry?.contact?.id) {
      console.info(
        '[moneybird] contact_created_without_email',
        JSON.stringify({
          moneybirdContactId: String(retry.contact.id),
          hadEmailOnFirstAttempt: true,
          reason: 'email_domain_unreachable_retry',
        })
      );
      return {
        contactId: String(retry.contact.id),
        matched: false,
        created: true,
    ...
    return {
      ...
      matched: false,
      created: false,
      reason: retry?.reason || created?.reason || 'moneybird_contact_create_failed',
      errorStatus: retry?.errorStatus ?? created?.errorStatus ?? null,
      errorMessage: retry?.errorMessage ?? created?.errorMessage ?? null,
      errorDetails: retry?.errorDetails ?? created?.errorDetails ?? null,
    };
  }
  return {
    contactId: null,
    ...
    errorStatus: created?.errorStatus ?? null,
    errorMessage: created?.errorMessage ?? null,
    errorDetails: created?.errorDetails ?? null,
  };
}
...
export async function listContacts({ page = 1, perPage = 100, query = '' } = {}) {
  const p = Math.max(1, Number(page) || 1);
  const pp = Math.min(100, Math.max(1, Number(perPage) || 100));
  const q = normalizeText(query);
  const qs = [`page=${encodeURIComponent(String(p))}`, `per_page=${encodeURIComponent(String(pp))}`];
  if (q) qs.push(`query=${encodeURIComponent(q)}`);
  const path = `/contacts.json?${qs.join('&')}`;
  const data = await mbRequest(path, { method: 'GET' });
