...
// WhatsApp gaat via GHL-workflows (templates); geen Conversations-API-berichten meer vanuit deze app.
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    return res.status(410).json({
      error:
        'POST WhatsApp-debug is verwijderd: deze app stuurt geen WhatsApp meer via de GHL Conversations API. Gebruik GET voor health; test templates in GHL workflows.',
    });
  }

  if (req.method !== 'GET') {
    return res.status(405).json({ error: 'Gebruik GET voor health.' });
  }
